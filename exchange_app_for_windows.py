...
def exchange(current_currency_amount: float,current_currency:str, wanted_currency:str) -> str:
    ...
    try:
        url = f'https://v6.exchangerate-api.com/v6/66c0148aeceee70c30376892/latest/{current_currency.split()[0]}'
        response = requests.get(url)
        rate = response.json()["conversion_rates"][wanted_currency.split()[0]]
        current_result = rate * current_currency_amount
    except KeyError:
        return "Server error! Please try again later!"
    else:
        return (f"Current exchange rate:\n1 {current_currency.split()[0]} = {rate:.3f}{wanted_currency.split()[0]}\n\n"
                f"For {current_currency_amount:.3f} {current_currency.split()[0]}\n "
                f"you will receive {current_result:.3f}{wanted_currency.split()[0]}\n\n")
...
def toggle():
    ...
    if switch_value:
        ...
        my_canvas.create_text(450, 260, text="Choose the currency you want to exchange here: ", font=("Arial", 12),
                              fill="white")
        my_canvas.create_text(450, 320, text="Choose the currency you want to receive here:", font=("Arial", 12),
                              fill="white")
        my_canvas.create_text(450, 385, text="Currency amount you want to exchange: ", font=("Arial", 12), fill="white")
        my_canvas.create_text(800, 53, text="For more detailed currency information: ", font=("Arial", 12),
                              fill="white")
        ...
        dashboard_text.config(bg="black", fg="white", font=("Arial" , 12))
        response_field.config(bg="black", fg="white")
        my_canvas.create_text(790, 140, text="For 'light mode' click here:", font=("Arial", 12), fill="white")
        ...
        my_canvas.create_text(450, 320, text="Choose the currency you want to receive here:", font=("Arial", 12),
                              fill="black")
        my_canvas.create_text(450, 385, text="Currency amount you want to exchange: ", font=("Arial", 12), fill="black")
        my_canvas.create_text(800, 53, text="For more detailed currency information: ", font=("Arial", 12),
                              fill="black")
        ...
        dashboard_text.config(bg="light yellow", fg="black", font=("Arial", 12))
        response_field.config(bg="light blue", fg="black")
        my_canvas.create_text(770, 140, text="For 'dark mode' click here:", font=("Arial", 12), fill="black")
...
with open(resource_path("resources/currencies_data_for_windows.txt"), "r") as data_currency:
...
switch.config(text="Dark mode")
switch.place(x= 725, y = 150)

#set image in canvas
my_canvas.create_image(0,0,image = light, anchor="nw")

#create labels for input fields
my_canvas.create_text(450,260 , text="Choose the currency you want to exchange here: ",font=("Arial", 12), fill="black")
my_canvas.create_text(450,320 , text="Choose the currency you want to receive here:",font=("Arial", 12), fill="black")
my_canvas.create_text(450,385 , text="Currency amount you want to exchange: ",font=("Arial", 12), fill="black")
my_canvas.create_text(770,140 , text="For 'dark mode' click here:",font=("Arial", 12), fill="black")
...
#arrange menus
input_menu.place(x= 350, y= 275)
receive_menu.place(x= 350, y = 335)

#create amount entry field and arrange it
input_field_entry_amount = tk.Entry(my_canvas, width=13)
input_field_entry_amount.config(justify='center', bg="light blue", fg="black")
input_field_entry_amount.place(x= 410, y= 400)
...
dashboard_text.place(x= 320, y = 20)

#create button for pop-up informative window
my_canvas.create_text(800,53 , text="For more detailed currency information: ",font=("Arial", 12), fill="black")
informative_button = ttk.Button(my_canvas, width=15, text = " Click Here!", command= lambda: information_pop_up() )
informative_button.place(x= 715, y = 65)
